...
describe Structural do
  let(:point_class) { Structural.new(:x, :y) }
  ...
  it "requires all arguments to be present" do
    expect { point_class.new }.to raise_error(ArgumentError, "Expected 2 arguments, got 0")
  end

  it "can be given default values for arguments" do
    xpoint_class = point_class.with_defaults(y: 1)
  ...
  it "checks equality based on its attributes" do
    p1 = point_class.new(1, 2)
    p2 = point_class.new(1, 2)
    p3 = point_class.new(1, 3)
